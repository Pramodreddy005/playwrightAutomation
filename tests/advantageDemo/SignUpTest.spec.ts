import { expect, test } from "../baseSetup.spec";
...
test("SignInTest",async ({page}) => {
    ...
    await page.locator("input[name='username']").fill("hiuser441");
    await page.locator("input[name='password']").fill("Hiuser@123");
    await page.locator("input[name='remember_me']").check();
    await page.getByRole('button',{name:'SIGN IN'}).click();    
    await page.locator('.spinner').waitFor({ state: 'hidden', timeout: 60000 });
    await expect( page.locator("svg[id='menuUser']")).toBeVisible({timeout:6000});
    await page.locator("svg[id='menuUser']").click();
    expect(await page.locator("a[id='menuUserLink']>span").innerText()).toBe("hiuser441");
});
